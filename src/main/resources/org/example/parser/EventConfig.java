 ...
 * <p>
 * @param maxEventQueueForCons max events that can be put in the forCons queue (q2) in ConsensusRoundHandler
 *                             (0 for infinity)
 * @param eventIntakeQueueThrottleSize The value for the event intake queue at which the node should stop syncing
 * @param eventIntakeQueueSize The size of the event intake queue, {@link QueueThreadConfiguration#UNLIMITED_CAPACITY}
 *                             for unbounded. It is best that this queue is large, but not unbounded. Filling it up can
 *                             cause sync threads to drop TCP connections, but leaving it unbounded can cause out
 *                             of memory errors, even with the {@link #eventIntakeQueueThrottleSize}, because syncs
 *                             that started before the throttle engages can grow the queue to very large sizes on
 *                             larger networks.
 * @param randomEventProbability The probability that after a sync, a node will create an event with a random other
 *                               parent. The probability is is 1 in X, where X is the value of randomEventProbability.
 *                               A value of 0 means that a node will not create any random events.
 *                               <p>
 *                               This feature is used to get consensus on events with no descendants which are created
 *                               by nodes who go offline.
 * @param staleEventPreventionThreshold A setting used to prevent a node from generating events that will probably
 *                                      become stale. This value is multiplied by the address book size and compared to
 *                                      the number of events received in a sync.
 *                                      If ({@code numEventsReceived > staleEventPreventionThreshold * addressBookSize})
 *                                      then we will not create an event for that sync, to reduce the probability of
 *                                      creating an event that will become stale.
 ...
 * @param eventStreamQueueCapacity capacity of the blockingQueue from which we take events and write to EventStream files
